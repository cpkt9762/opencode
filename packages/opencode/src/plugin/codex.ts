...
export async function fetchCodexUsage(
  account: Auth.CodexAccount,
  multi?: Auth.CodexMultiAccount,
): Promise<Auth.CodexUsage> {
  if (!account.access || account.expires < Date.now()) {
    ...
    if (multi) await Auth.codexSave(multi)
  }
...
async function codexFetch(
  account: Auth.CodexAccount,
  multiAuth: Auth.CodexMultiAccount,
  ...
  if (!account.access || account.expires < Date.now()) {
    ...
    account.expires = Date.now() + (tokens.expires_in ?? 3600) * 1000
    account.accountId = extractAccountId(tokens) ?? account.accountId
    await Auth.codexSave(multiAuth)
  }
  ...
  if (primary || secondary) {
    ...
    Auth.codexSave(multiAuth).catch(() => {})
  }
  ...
  return {
    auth: {
      ...
      async loader(getAuth, provider) {
        const multiAuth = await Auth.codex()
